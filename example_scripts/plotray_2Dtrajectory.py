...
def plotray2D(dates, fs, bs):
    ...
    for cdate, cf, bsstr in zip(dates, fs, bs):
        ...
        datadir = datadir + rename + '/' + str(freq[0]/1e3) + 'kHz' + rename + str(ray_datenum.hour) + str(ray_datenum.minute) + '/plots/'
        try:
            os.mkdir(datadir)
        except OSError:
            pass
        ...
        # get MLT
        mlt = aacgmv2._aacgmv2.mlt_convert(ray_datenum.year, ray_datenum.month, ray_datenum.day, ray_datenum.hour, ray_datenum.minute, ray_datenum.second, outsph_foot.long)
        mltmin = (mlt % 1) * 60 
        mlt = str(math.floor(mlt)) + ':' + str(round(mltmin))
